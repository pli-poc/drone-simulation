...
passed=biology['available'] and unit['available'] and unit['tests']>0 and unit['failures']==0 and unit['errors']==0 and browser['available'] and browser['passed'] and browser['tests']>0 and benchmark['available'] and os.environ.get('VERIFY_STATUS','success')=='success'
report={'schema':'mosquito-drone-lab/verification@1','commit':os.environ.get('GITHUB_SHA','local'),'run':os.environ.get('GITHUB_RUN_ID','local'),'attempt':os.environ.get('GITHUB_RUN_ATTEMPT','1'),'ref':os.environ.get('GITHUB_REF','local'),'event':os.environ.get('GITHUB_EVENT_NAME','local'),'created':datetime.datetime.now(datetime.timezone.utc).isoformat(),'passed':bool(passed),'unit':unit,'browser':browser,'benchmark':benchmark,'biology':biology,'limitations':'Software verification only; not biological or physical safety validation.'}
