...
setInterval(()=>{const now=performance.now();accumulator+=running?Math.min(0.2,(now-last)/1000)*rate:0;last=now;let moved=false;while(accumulator>=STEP){advance();accumulator-=STEP;moved=true;}if(moved)emit();},30);
emit();
