...
import {Learner,Network} from '../learning/dqn.js';
import {config} from '../core/world.js';
let generation=0;
const yieldUI=()=>new Promise(resolve=>setTimeout(resolve,0));
self.onmessage=({data:m})=>{
  if(m.type==='cancel'){generation++;postMessage({type:'cancelled'});}
  if(m.type==='train'){const token=++generation;train(m,token).catch(e=>postMessage({type:'error',message:e.message}));}
};
async function evaluate(c,model,token){
  const results=[];
  for(let k=0;k<12;k++){
    if(token!==generation)return null;
    const env=new Environment({...c,seed:700001+k*7919});
    while(!env.done){env.step(model?model.action(env.observation()):0);if(env.metrics.steps%60===0){await yieldUI();if(token!==generation)return null;}}
    results.push({seed:env.config.seed,...env.metrics,outcome:env.outcome});await yieldUI();
  }
...
async function train(m,token){
  ...
  for(let ep=0;ep<episodes;ep++){
    ...
    const env=new Environment({...c,seed:c.seed+ep*997});let state=env.observation();
    const epsilon=Math.max(0.06,0.4*Math.pow(0.96,ep));
    while(!env.done){
      const action=learner.choose(state,epsilon),r=env.step(action);learner.remember(state,action,r.reward,r.observation,r.terminated||r.truncated);state=r.observation;
      if(env.metrics.steps%40===0){await yieldUI();if(token!==generation)return;}
    }
  ...
  postMessage({type:'evaluating',message:'Evaluating frozen policies on 12 held-out seeds each…'});
  const baseline=await evaluate(c,null,token);if(!baseline)return;
  const learned=await evaluate(c,learner.net,token);if(!learned)return;
