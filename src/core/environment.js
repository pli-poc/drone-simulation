...
export class Environment{
  ...
  step(action=0){
    ...
    const c=this.config,goal=c.task==='navigation'?navGoal(this.time):this.insect.p,oldDistance=distance(this.drone.p,goal);this.lastAction=action;
    this.safety=filter(this.drone,propose(this.drone,this.track,this.sensors.ranges,c,this.time,action),this.boxes(),c,this.track);
    ...
    for(let i=0;i<6;i++){
      ...
      const a=limit(mul(sub(this.safety.command,this.drone.v),5),4);this.drone.v=limit(add(this.drone.v,mul(a,PHYSICS_STEP)),c.speed);
      ...
      if(!this.done&&c.task==='contact'&&!protectedContact&&!this.safety.protectedTarget&&this.track?.confidence>0.2&&segmentOriginDistance(sub(oldHead,oldTarget),sub(this.head(),this.insect.p))<0.072){this.metrics.contacts++;this.done=true;this.outcome='contact';}
      this.track=this.sensors.deliver(this.time);if(this.done)break;
    }
    this.sensors.sample(this.time,this.drone,this.insect,this.boxes(),c);this.track=this.sensors.deliver(this.time);
    if(this.metrics.clearance<0.08)this.metrics.nearMissSteps++;
    this.metrics.steps++;if(this.track)this.metrics.trackingSteps++;
    const d=distance(this.drone.p,c.task==='navigation'?goal:this.insect.p);
    // Ground truth is used in reward shaping, NOT in the learned policy's observations.
