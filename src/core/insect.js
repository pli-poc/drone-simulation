...
/** Procedural reactive surrogate with idealized proximity cues. Not flybody or species-calibrated. */
export class ReactiveInsect{
  ...
  step(dt,drone,boxes,c,time){
    ...
    for(const axis of AXES){let r=1;for(const b of boxes)r=Math.min(r,rayBox(this.p,axis,b,0,1));if(r<0.45)drive=add(drive,mul(axis,-(0.45-r)*6));}
    const wind=[c.wind*Math.sin(time*0.7),c.wind*0.12*Math.cos(time),c.wind*0.5*Math.cos(time*0.43)];
    if(d<0.6)wind[1]-=0.18*(1-d/0.6); // Cheap disturbance surrogate, not CFD.
