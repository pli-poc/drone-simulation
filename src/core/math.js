...
export function boxDistance(p,b) {
  return Math.hypot(...p.map((v,i)=>Math.max(Math.abs(v-b.p[i])-b.size[i]/2,0)));
}
/** Ray/slab intersection. dir need not be normalized; returned value is its parameter. */
...
export function segmentOriginDistance(a,b) {
  const d=sub(b,a); const t=clamp(-dot(a,d)/(dot(d,d)||1),0,1); return norm(add(a,mul(d,t)));
}
