...
export const DEFAULTS=Object.freeze({scenario:'home',seed:42,speed:1.35,range:5.5,latency:80,agility:0.65,wind:0.12,dropout:0.04,duration:24,task:'contact',insectModel:'procedural',bioFrequency:218,bioMassScale:1,neuralDtMs:0.5});
export function config(input={}){
  ...
  if(['procedural','biological','neural'].includes(input.insectModel))c.insectModel=input.insectModel;
  if([0.1,0.2,0.5,1].includes(input.neuralDtMs))c.neuralDtMs=input.neuralDtMs;
...
export function obstacles(world,t){return world.kind!=='dynamic'?world.static:[...world.static,box('occupant',[Math.sin(t*0.55)*1.6,0.9,0.25],[0.5,1.8,0.5],'person')];}
export function navGoal(t){return [[-0.8,1.65,-0.8],[2.15,2.1,-0.2],[1.1,1.7,2.2],[-2.6,1.8,1.6]][Math.floor(t/5)%4];}
