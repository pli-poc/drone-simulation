...
export class Network{
  constructor(seed=123){const r=new RNG(seed);this.w1=Array.from({length:lengths.w1},()=>r.signed()*0.15);this.b1=Array(HIDDEN).fill(0);this.w2=Array.from({length:lengths.w2},()=>r.signed()*0.03);this.b2=Array(OUTPUTS).fill(0);}
  forward(x){
    if(x.length!==INPUTS||x.some(v=>!Number.isFinite(v)))throw new Error('Expected 19 finite observations.');
    ...
    for(let a=0;a<OUTPUTS;a++){let s=this.b2[a];for(let j=0;j<HIDDEN;j++)s+=this.w2[a*HIDDEN+j]*h[j];q[a]=s;}return {h,q};
  }
...
/** Small transparent DQN baseline. No optimizer state is persisted: imports are weight warm starts. */
export class Learner{
  ...
  optimize(){
    ...
    for(let b=0;b<8;b++){
      const e=this.memory[Math.floor(this.rng.next()*this.memory.length)],{h,q}=this.net.forward(e.s),target=clamp(e.r+(e.done?0:0.97*Math.max(...this.target.forward(e.next).q)),-30,30);
      ...
      for(let j=0;j<HIDDEN;j++){this.net.w2[e.a*HIDDEN+j]-=lr*(delta*h[j]+1e-5*this.net.w2[e.a*HIDDEN+j]);const dh=delta*old[j]*(1-h[j]*h[j]);for(let i=0;i<INPUTS;i++)this.net.w1[j*INPUTS+i]-=lr*(dh*e.s[i]+1e-5*this.net.w1[j*INPUTS+i]);this.net.b1[j]-=lr*dh;}
